...
public class RongManger {

	private static final Logger log = LoggerFactory.getLogger(RongManger.class);
	public static StringBuilder EXISTS_JID = new StringBuilder();
	public static StringBuilder EXISTS_MANAGER = new StringBuilder();
	public static StringBuilder ROOM_MEMBERS = new StringBuilder();
	public static StringBuilder INSERT_MEMBER = new StringBuilder("insert into ofmucmember(roomID,jid,nickName) values(? , ? , ?)");
	static{
		...
		EXISTS_MANAGER.append(" select ofmucroom.serviceID, ofmucroom.name, ofmucroom.roomid ,null from ");
		EXISTS_MANAGER.append(" ofmucroom  join ofmucaffiliation on ofmucroom.roomID = ofmucaffiliation.roomID and ofmucaffiliation.jid = ? and ofmucroom.name=?");
	 ...
	 * @return
	 */
	public static boolean existsManager(String jid,String roomName) {
		Connection connection = null;
		PreparedStatement statement = null;
		ResultSet resultSet = null;
		try {
			connection = DbConnectionManager.getConnection();
			statement = connection.prepareStatement(EXISTS_MANAGER.toString());
			statement.setString(1, jid);
			statement.setString(2, roomName);
			resultSet = statement.executeQuery();
